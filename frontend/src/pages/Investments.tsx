...
function txnTypeStyle(type: string): string {
  switch (type) {
    ...
    case 'dividend': return 'bg-emerald-500/10 text-emerald-400';
    ...
    <div>
      ...
      {holdings.length > 0 && (
        <div className="mb-6 rounded-2xl border border-neutral-800 bg-neutral-900 p-5">
          <div className="flex flex-wrap gap-6">
            ...
            {totalCostBasis > 0 && (
              <div>
                <p className="text-xs text-neutral-500">Cost Basis</p>
                <p className="text-2xl font-bold text-neutral-400">{currencyFmt.format(totalCostBasis)}</p>
              </div>
            )}
      ...
      <div role="tablist" className="mb-4 flex gap-1">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
            tab === 'holdings' ? 'bg-neutral-800 text-white' : 'text-neutral-400 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
            tab === 'transactions' ? 'bg-neutral-800 text-white' : 'text-neutral-400 hover:text-white'
          }`}
        >
        ...
        <div role="tabpanel">
          {holdings.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-neutral-700 py-16 text-center">
              <p className="text-lg text-neutral-400">No holdings yet</p>
              <p className="mt-1 text-sm text-neutral-500">Link an investment or crypto account to see holdings</p>
            </div>
          ) : (
            <>
              {/* Desktop table */}
                    ...
                    {holdings.map(h => {
                      ...
                      return (
                        <tr key={h.id} className="hover:bg-neutral-800/40">
                          ...
                          <td className="px-4 py-3 text-neutral-400">{h.name || '\u2014'}</td>
                          <td className="px-4 py-3 text-right text-neutral-300">{formatQuantity(h.quantity)}</td>
                          ...
                          <td className={`px-4 py-3 text-right font-medium ${gl ? (gl.value >= 0 ? 'text-green-400' : 'text-red-400') : 'text-neutral-500'}`}>
                            {gl ? `${gl.value >= 0 ? '+' : '\u2212'}${currencyFmtCents.format(Math.abs(gl.value))} (${gl.pct >= 0 ? '+' : ''}${gl.pct.toFixed(1)}%)` : '\u2014'}
                          </td>
                          <td className="px-4 py-3 text-neutral-400">{h.accountName}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                    ...
                    <div key={h.id} className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          {!h.ticker && (
                            <p className="text-xs text-neutral-500">{formatQuantity(h.quantity)} units</p>
                          )}
                        </div>
      ...
      {tab === 'transactions' && (
        <div role="tabpanel">
          {txns.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-neutral-700 py-16 text-center">
              <p className="text-lg text-neutral-400">No investment transactions yet</p>
            </div>
          ) : (
            <>
              <div className="hidden overflow-x-auto rounded-xl border border-neutral-800 md:block">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-neutral-800">
                    {txns.map(txn => (
                      <tr key={txn.id} className="hover:bg-neutral-800/40">
                        <td className="whitespace-nowrap px-4 py-3 text-neutral-400">{formatDate(txn.date)}</td>
                        ...
                        <td className="px-4 py-3 text-neutral-400">{txn.accountName}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-neutral-500">
                          ...
                          <span className={`capitalize ${
                            ...
                            txn.type === 'dividend' ? 'text-emerald-400' :
                            'text-neutral-400'
                          }`}>{txn.type}</span>
